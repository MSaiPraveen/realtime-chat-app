...
const ChatRoom = ({ roomId, onRoomChange }) => {
  ...
  useEffect(() => {
    ...
    const getRoomName = async () => {
      try {
        ...
        // Note: This assumes room name is stored in metadata, update as needed
    ...
    // Query messages in this specific room
    const q = query(
  ...
  const saveEditMessage = async () => {
    ...
    try {
      const msgRef = doc(db, `rooms/${roomId}/messages`, editingId);
  ...
  return (
    <div className="flex h-screen w-full bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600">
      {/* Room Manager Sidebar */}
      <RoomManager onSelectRoom={onRoomChange} selectedRoomId={roomId} />
      ...
      <div className="flex-1 flex flex-col">
        {!roomId ? (
          ...
          <div className="flex-1 flex items-center justify-center flex-col">
            ...
            <p className="text-white/70 text-lg">Create or select a room to start chatting</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <header className="bg-white/5 backdrop-blur-xl border-b border-white/10 px-6 py-4 shadow-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl font-bold text-white">{roomName || 'Chat Room'}</h1>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-white/70 text-lg">Start the conversation!</p>
                </div>
              ) : (
                messages.map(msg => (
                  <div
